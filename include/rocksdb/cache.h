...
// Create a new CacheLib cache with a fixed size capacity. The cache is not sharded
// to 2^num_shard_bits shards, by hash of the key ?? TODO do we want shared (pools in 
//  cachelib speak). The total capacity
// is divided and evenly assigned to each shard. If strict_capacity_limit
// is set, insert to the cache will fail when cache is full. User can also
// set percentage of the cache reserves for high priority entries via
// high_pri_pool_pct.
